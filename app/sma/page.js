...
const kelasList = [
  { id: 10, label: "Kelas 10", icon: "🔟", desc: "Pengenalan mapel peminatan" },
  { id: 11, label: "Kelas 11", icon: "1️⃣1️⃣", desc: "Pendalaman materi peminatan" },
  { id: 12, label: "Kelas 12", icon: "1️⃣2️⃣", desc: "Persiapan UTBK & ujian akhir", ujianAkhir: true },
];

const mapelList = [
  { id: "matematika", label: "Matematika", icon: "🔢", color: "#8E44AD" },
  { id: "fisika", label: "Fisika", icon: "⚡", color: "#2980B9" },
  { id: "kimia", label: "Kimia", icon: "🧪", color: "#27AE60" },
  { id: "biologi", label: "Biologi", icon: "🧬", color: "#E67E22" },
  { id: "ekonomi", label: "Ekonomi", icon: "📈", color: "#E74C3C" },
  { id: "bind", label: "B. Indonesia", icon: "📝", color: "#1B3A6B" },
  { id: "bing", label: "B. Inggris", icon: "🌐", color: "#D4A017" },
];

const paketSoal = [
  { id: 1, judul: "Latihan Harian 1", desc: "Soal latihan harian sesuai kisi-kisi MGMP daerahmu", soal: 30, waktu: 45, harga: 10000, badge: "BARU", badgeColor: "#27AE60", emoji: "📝", siswa: 423, filter: ["terbaru","uts"] },
  { id: 2, judul: "Latihan Harian 2", desc: "Variasi soal berbeda dari Part 1, tingkat sedang", soal: 30, waktu: 45, harga: 10000, badge: null, badgeColor: null, emoji: "📝", siswa: 356, filter: ["terbaru"] },
  { id: 3, judul: "Latihan Harian 3", desc: "Soal latihan harian tingkat kesulitan lebih tinggi", soal: 30, waktu: 45, harga: 10000, badge: null, badgeColor: null, emoji: "📝", siswa: 289, filter: ["terbaru"] },
  { id: 4, judul: "Simulasi UTS", desc: "Simulasi Ujian Tengah Semester sesuai kisi-kisi daerahmu", soal: 45, waktu: 75, harga: 10000, badge: "🔥 POPULER", badgeColor: "#E67E22", emoji: "📋", siswa: 2134, filter: ["terpopuler","uts"] },
  { id: 5, judul: "Simulasi UAS", desc: "Simulasi Ujian Akhir Semester sesuai kisi-kisi daerahmu", soal: 50, waktu: 90, harga: 10000, badge: "🔥 POPULER", badgeColor: "#E67E22", emoji: "📋", siswa: 1876, filter: ["terpopuler","uas"] },
  { id: 6, judul: "Simulasi UTBK", desc: "Persiapan UTBK/SNBT — soal sesuai standar SNPMB terbaru", soal: 60, waktu: 90, harga: 10000, badge: "UTBK", badgeColor: "#8E44AD", emoji: "🎯", siswa: 3241, filter: ["terpopuler","uas"] },
  { id: 7, judul: "Bundling Lengkap", desc: "Semua paket dalam satu — latihan harian + UTS + UAS + UTBK", soal: 200, waktu: 180, harga: 35000, badge: "HEMAT 50%", badgeColor: "#E74C3C", emoji: "🎁", siswa: 987, filter: ["terpopuler","uts","uas"] },
];
...
const kisiKisiDummy = [
  { bab: "Bab 1", judul: "Fungsi dan Limit", kompetensi: "Memahami konsep fungsi, komposisi, dan limit fungsi", indikator: ["Menentukan domain dan range fungsi", "Menghitung komposisi fungsi", "Menyelesaikan limit fungsi aljabar"] },
  { bab: "Bab 2", judul: "Turunan dan Aplikasinya", kompetensi: "Memahami konsep turunan dan penerapannya", indikator: ["Menentukan turunan fungsi aljabar", "Menerapkan turunan dalam masalah nyata", "Menentukan nilai ekstrim fungsi"] },
  { bab: "Bab 3", judul: "Integral", kompetensi: "Memahami konsep integral dan aplikasinya", indikator: ["Menghitung integral tak tentu", "Menghitung integral tentu", "Menerapkan integral untuk luas daerah"] },
];
...
export default function SMA() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "#F0F2F7" }}>
          ...
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            {!showTimerSetup ? (
              <>
                ...
                <div style={{ background: "#F5EEF8", borderRadius: 10, padding: "10px 14px", marginBottom: 14, textAlign: "center" }}>
              ...
              <>
                ...
                <div className="grid-2" style={{ marginBottom: 20 }}>
                  {[
                    ...
                    <div key={t.mode} onClick={() => setTimerMode(t.mode)} style={{ border: `2px solid ${timerMode === t.mode ? WARNA : "#E2E8F0"}`, borderRadius: 14, padding: 16, cursor: "pointer", background: timerMode === t.mode ? "#F5EEF8" : "white", textAlign: "center", transition: "all 0.2s" }}>
      ...
      {/* HEADER */}
      <div style={{ background: `linear-gradient(135deg, ${WARNA}, #6C3483)`, padding: "32px 20px 44px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
          ...
          <div>
            <div style={{ display: "inline-block", background: "rgba(255,255,255,0.25)", color: "white", fontSize: 12, fontWeight: 800, padding: "4px 14px", borderRadius: 50, marginBottom: 8 }}>SEKOLAH MENENGAH ATAS</div>
            <h1 className="hero-title" style={{ color: "white", marginBottom: 4 }}>Latihan Soal SMA</h1>
            <p style={{ color: "rgba(255,255,255,0.85)", fontSize: 14 }}>Kelas 10–12 · Semua Mata Pelajaran · Kisi-kisi MGMP Lokal</p>
          </div>
        </div>
      </div>
            ...
            <div className="grid-3">
              {kelasList.map((k) => (
                <div key={k.id} onClick={() => { setKelas(k); setMapel(null); }} style={{ background: kelas?.id === k.id ? WARNA : "#F8F9FA", borderRadius: 14, padding: "16px 10px", cursor: "pointer", border: `2px solid ${kelas?.id === k.id ? WARNA : "transparent"}`, textAlign: "center", transition: "all 0.2s", position: "relative" }}>
                  ...
                  <div style={{ fontSize: 10, color: kelas?.id === k.id ? "rgba(255,255,255,0.85)" : "#9AA5B4", marginTop: 4 }}>{k.desc}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {kelas && (
          <div className="card">
            ...
            <div className="grid-4">
              {mapelList.map((m) => (
                <div key={m.id} onClick={() => setMapel(m)} style={{ background: mapel?.id === m.id ? m.color : "#F8F9FA", borderRadius: 14, padding: "14px 8px", cursor: "pointer", border: `2px solid ${mapel?.id === m.id ? m.color : "transparent"}`, textAlign: "center", transition: "all 0.2s" }}>
                  <div style={{ fontSize: 24, marginBottom: 6 }}>{m.icon}</div>
                  <div style={{ fontWeight: 800, fontSize: 11, color: mapel?.id === m.id ? "white" : "#1B3A6B" }}>{m.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            {activeTab === "kisiKisi" && (
              <div>
                ...
                <div style={{ background: "#F5EEF8", border: `1px solid ${WARNA}`, borderRadius: 14, padding: 16, textAlign: "center" }}>
